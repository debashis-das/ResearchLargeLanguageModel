...
class MultiGPUExecutor:
  ...
  def __init__(self, world_size, rank, device=DEVICE):
    ...
    self.mlp = MLP(Config.hiddens, Config.mlp_intermediate_hidden)
  ...
  def execute(self):
    try:
      ...
      self.transformerPerGPU(sample_input)
  ...
  def transformerPerGPU(self, sample_input):
      ...
      for _ in range(Config.learning_iter):
        ...
        output = self.attention(q, k, v, block_m, block_n, Config.num_head, n_ctx, Config.hiddens, Config.sm_scale, DEVICE, self.rank, self.rank)
