        ...
        for i in range(2):
            ...
            for _, row in df_shuffled.iterrows():
                ...
                if training_timestep % 10 == 0:
                    ...
                    generation_ids = model_with_lora.generate(input_ids_rl, max_new_tokens=100, temperature=0.7)
...
def sft_execute():
    try:
        ...
        input_ids_rl = torch.tensor(row_rl['input_ids'], dtype=torch.long, device=device).unsqueeze(0)
        attention_mask_rl = torch.tensor(row_rl['attention_mask'], dtype=dtype, device=device).unsqueeze(0)
        output_ids = model_with_lora.generate(input_ids_rl, max_new_tokens=100, temperature=0.7)
