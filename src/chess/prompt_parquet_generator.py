...
CHESS_SYSTEM_PROMPT = """Chess is a two-player abstract strategy board game played on an 8x8 grid (64 squares). Each player controls 16 pieces: one king, one queen, two rooks, two bishops, two knights, and eight pawns. White moves first, and the goal is to checkmate the opponent's king (threaten it with inescapable capture).
Piece Movement
...
Your task is to predict the next set of moves for both black and white till you win the game.
The initial moves will be provided in the following format:
...
IMPORTANT OUTPUT FORMAT: Respond with ONLY the continuation of moves in SAN notation (move number, dot, white move, black move, repeated as needed — e.g. "13. Nxd8 Kxd8 14. Bc4 Be6"). Do not include any explanations, commentary, headers, markdown formatting, or any text other than the moves themselves."""
...
def prompt_generator(chess_json, training_type):
    ...
    user_content = f'play_as: {chess_json["play_as"]}\nmoves: {extracted_moves.strip()}'
    prompt_messages = [
        {"role": "system", "content": CHESS_SYSTEM_PROMPT},
        {"role": "user", "content": user_content},
    ]
    if training_type == TrainingType.SUPERVISED_LEARNING:
        return prompt_messages, generate_moves.strip()
    ...
    with bz2.open(file_path, 'rt', encoding='utf-8') as file:
        for i, line in enumerate(file):
            ...
            if len(line) > 0 and not line.startswith("[") and "eval" not in line:  # Limit the number of lines printed
                ...
                try:
                    prompt_messages, target_text = prompt_generator(chess_format_dic, training_type)
                    if training_type == TrainingType.SUPERVISED_LEARNING:
                        # prompt_ids marks where the model's own turn starts — everything before this
                        # index is given context (system rules + game history), not something the
                        # model had to predict, so it's excluded from the loss at training time.
                        prompt_ids = tokenizer.apply_chat_template(prompt_messages, tokenize=True, add_generation_prompt=True)["input_ids"]
                        full_messages = prompt_messages + [{"role": "assistant", "content": target_text}]
                        full_ids = tokenizer.apply_chat_template(full_messages, tokenize=True, add_generation_prompt=False)["input_ids"]
                        if len(prompt_ids) >= max_sequence_length or len(full_ids) > max_sequence_length:
                            raise ValueError("Tokenized sequence exceeds max_sequence_length")
                        prompt_length = len(prompt_ids)
                        pad_amount = max_sequence_length - len(full_ids)
                        attention_mask = [1] * len(full_ids) + [0] * pad_amount
                        full_ids = full_ids + [pad_id] * pad_amount
                        df.loc[len(df)] = [
                            torch.tensor(full_ids, dtype=torch.long).numpy(),
                        ...
                        df.loc[len(df)] = [
                            torch.tensor(input_ids, dtype=torch.long).numpy(),
                            torch.tensor(attention_mask, dtype=torch.long).numpy(),
                            len(input_ids),
                        ]
                    counter += 1
