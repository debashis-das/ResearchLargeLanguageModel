...
def rl_train(load_path = ""):
    try:
        ...
        grpo_reward_model = GRPORewardModel(tokenizer, model_with_lora, grpo_batch=2, model_device="cuda:1", loss_device="cuda:0", dtype=dtype)
        training_timestep = 0
        recover = False
        for i in range(1,2):
            if recover:
            ...
            df_shuffled = df_input.sample(frac=1, ignore_index=True, random_state=3278)
